...
function clientCarosule () {
	if ($('#certification .owl-carousel').length) {
...
function stickyHeader () {
	var headerScrollfixed = $(window).scrollTop();
	// console.log(headerScrollfixed);
	if(headerScrollfixed > 120) {
	...
	if ($('.custom-file-input').length ) {
		$(".custom-file-input").on("change", function() {
			...
			if (fileName.length) {
				...
				$(this).siblings(".custom-file-label").addClass("selected").html('+ Add Attachment');
			}
		});
	}
}
...
function scrollmenu () {
	if ($('.mainmenu, .scrolink').length ) {
		$('.mainmenu a.scroll, .scrolink').click(function(e){        
			e.preventDefault();
			// header(); 
			var id = $(this).attr('href');
			if (id.length!=0) {
				if ($('.header-curvy').hasClass('header-fixed')){
					$('html, body').animate({
...
function s_time () {
	...
	$('.g-product-card:not(.g-large-product)').on('click', function(){
		var this_product = $(this).html();
		var large_product = $(this).closest('section').find('.g-product-card.g-large-product').html();
		$(this).html(large_product);
		$(this).closest('section').find('.g-product-card.g-large-product').hide().html(this_product).fadeIn(500);
	});
